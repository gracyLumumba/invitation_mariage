...
async function nextInviteCode() {
  for (let i = 1; i < 10000; i += 1) {    const code = `TL${String(i).padStart(2, '0')}`;
...
// GET /valider-entree?code=TF01&table=5
...
async function startServer() {
  ...
  if (USE_HTTPS) {
    ...
    https.createServer(credentials, app).listen(PORT, BIND_HOST, () => {
      console.log('\n[DEMARRAGE] Système mariage Tresor & Laurette démarré en HTTPS !');
    ...
    app.listen(PORT, BIND_HOST, () => {
      console.log('\n[DEMARRAGE] Système mariage Tresor & Laurette démarré !');
